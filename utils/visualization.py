...
def plot_tracking_select(image, tlwhs, obj_ids, scores=None, frame_id=0, fps=0., ids2=None):#select press s normal press num select
    ...
    for i, tlwh in enumerate(tlwhs):
        ...
        keyName2 = cv2.waitKey(50)
        #keyName3= cv2.waitKey(10)
        #test=obj_id
       # if(keyName2 == ord(test)):
            #if (keyName3 != ord(id_text)):
           # cv2.rectangle(im, intbox[0:2], intbox[2:4], color=color, thickness=line_thickness)
        #logger.info('text id: {}'.format(id_text))
        cancel = False
...
def plot_tracking_cancel(image, tlwhs, obj_ids, scores=None, frame_id=0, fps=0., ids2=None):#press num to cancel object
    im = np.ascontiguousarray(np.copy(image))
    im_h, im_w = im.shape[:2]
